...
document.getElementById('motTrouve').innerHTML = localStorage.getItem('victoire');
document.getElementById('motJouer').innerHTML = localStorage.getItem('motjouer');
document.getElementById('coupMoyen').innerHTML = localStorage.getItem('coupMoyen');
...
while (leMot.length < 5 || leMot.length >8 || leMot.indexOf('é') >-1 || leMot.indexOf('á') >-1 || leMot.indexOf('à') >-1 || leMot.indexOf('ã') >-1 || leMot.indexOf('â') >-1 || leMot.indexOf('è') >-1 || leMot.indexOf('ê') >-1 || leMot.indexOf('í') >-1 || leMot.indexOf('ì') >-1 || leMot.indexOf('î') >-1 || leMot.indexOf('õ') >-1 || leMot.indexOf('ó') >-1 || leMot.indexOf('ò') >-1 || leMot.indexOf('ô') >-1 || leMot.indexOf('ú') >-1 || leMot.indexOf('ù') >-1 || leMot.indexOf('û') >-1 || leMot.indexOf('ï') >-1 || leMot.indexOf('æ') >-1 || leMot.indexOf('œ') >-1 || leMot.indexOf('ü') >-1 || leMot.indexOf('ç') >-1 || leMot.substring(leMot.length-3, leMot.length) == "ons" || leMot.substring(leMot.length-2, leMot.length) == "ez" || leMot.substring(leMot.length-5, leMot.length) == "aient" || leMot.substring(leMot.length-3, leMot.length) == "ent" || leMot.substring(leMot.length-1, leMot.length) == 'a' || leMot.substring(leMot.length-2, leMot.length) == 'ai' || leMot.substring(leMot.length-3, leMot.length) == "ais" || leMot.substring(leMot.length-3, leMot.length) == "ait" || leMot.substring(leMot.length-1, leMot.length) == "o" || leMot.substring(leMot.length-1, leMot.length) == 's' || leMot.substring(leMot.length-3, leMot.length) == 'ont') {
...
document.addEventListener("keydown", async function (event) {
  ...
  if (event.keyCode == 13) {
    ...
    if(await document.getElementById(`${essaiActuel}-${wordLength}`).value.length == 1){
     ...
     while( i < tousLesMots.length && !trouve){
        ...
        MotExiste = tousLesMots[i].normalize("NFD").replace(/[\u0300-\u036f]/g, "")
      ...
      if(trouve){
        ...
        for(let x=1; x< wordLength+1; x++){
          ...
          MotExiste = leMotComplet[x-1].normalize("NFD").replace(/[\u0300-\u036f]/g, "")

          //Si la lettre du mot à trouver est égale à la lettre saisie
          if(await MotExiste.toUpperCase() ==  document.getElementById(`${essaiActuel}-${x}`).value)
          {
            lettreCorrectes += document.getElementById(`${essaiActuel}-${x}`).value; //Ajout lettre correcte dans la variable
            ...
            setTimeout(function declancherRouge(){
              document.getElementById(`${essaiActuel}-${x}`).style.backgroundColor = "#DF1300"; //Cahngement du fond en rouge
            let lettreInconnu = document.getElementById(`${essaiActuel}-${x}`).value; //prend la valeur de la lettre à giser
            document.getElementById(`${lettreInconnu.toLowerCase()}`).style.backgroundColor = "#DF1300"; //Lettre grisée en rouge
            document.getElementById(`${lettreInconnu}`).style.backgroundColor = "#DF1300"; //Lettre grisée en rouge
            }, secondes);      
          }
        }
        ...
        for(let a=1; a< wordLength+1; a++){
          ...
          lettresTrouvees += document.getElementById(`${essaiActuel}-${a}`).value;
               
          //Si la lettre de la ligne à déja été saisie plus tôt dans la ligne
          if((lettresTrouvees.split(document.getElementById(`${essaiActuel}-${a}`).value).length -1) != null){
            //Alors cpt prend le nombre de fois où la lettre existe dans la ligne
            cpt = (lettresTrouvees.split(document.getElementById(`${essaiActuel}-${a}`).value).length -1);
          }

          MotExiste = leMotComplet[a-1].normalize("NFD").replace(/[\u0300-\u036f]/g, ""); //Enleve accents
          let lettreParcourue = document.getElementById(`${essaiActuel}-${a}`).value;   //Prend la lettre de l'input que l'on parcours
     
          //Si la lettre parcourue existe moins ou autant de fois dans le mots de la ligne que dans le mot à deviner
          if((MotLigne.split(lettreParcourue).length -1) <= (leMotComplet.split(lettreParcourue).length -1)){

            //Si la lettre du mot à trouver est différente à la lettre saisie
            if(await MotExiste.toUpperCase() !=  document.getElementById(`${essaiActuel}-${a}`).value){

              let secondes = 250 * a; //Temps avant excution de la fonction
              //Délais avant d'afficher pour afficher petit à petit
              setTimeout(function declancherJaune(){
                document.getElementById(`${essaiActuel}-${a}`).style.backgroundColor = "#ffd000"; //Changement du fond en jaune
              }, secondes);
            } 
          }
          //Sinon, si le nombre de fois ou la lettre existe est inférieur ou égale au nombre où elle esxiste dans le mot à deviner
            ...
            if(lettreCorrectes.split(document.getElementById(`${essaiActuel}-${a}`).value).length -1 ==0){
              let lettreInconnu = document.getElementById(`${essaiActuel}-${a}`).value; //Prend la valuer à griser
              document.getElementById(`${lettreInconnu.toLowerCase()}`).style.backgroundColor = "grey"; //Lettre grisée en gris
              document.getElementById(`${lettreInconnu}`).style.backgroundColor = "grey"; //Lettre grisée en gris
            }
          }
        }                  
        ...
        if(nbLettresCorrect == wordLength){
          ...
          if(localStorage.getItem('coupMoyen') != 0){
            ...
            let moyenneCoup = (((Number(localStorage.getItem('victoire'))-1) * Number(localStorage.getItem('coupMoyen'))) + essaiActuel ) / Number(localStorage.getItem('victoire'));
            localStorage.setItem('coupMoyen', moyenneCoup.toFixed(1));
          }
          else{
            //Calcul du coup moyen pour trouver le bon mot
            let moyenneCoup = (((Number(localStorage.getItem('victoire'))-1) * 0) + essaiActuel ) / Number(localStorage.getItem('victoire'));
          ...
          setTimeout(async function victory(){
            ...
            document.getElementById('reponse').innerHTML = `Mot trouvé en ${essaiActuel} essai !`;
        ...
        //Défaite
         ...
         setTimeout(async function Defaite(){
          ...
          document.getElementById('motResult').innerHTML = `Le mot correct était : ${leMotComplet.toUpperCase()}`;
          document.getElementById('reponse').innerHTML = `Réesaye pour tenter de trouver le prochain mot !`;
          location.href = "#resultats";
         }, secondes);
        }
        //Sinon passe à la ligne suivante
         ...
         setTimeout(async function ligneSuivante(){
          ...
          leMotComplet = leMotComplet.normalize("NFD").replace(/[\u0300-\u036f]/g, "")//Enleve accent saisi

          document.getElementById(`${essaiActuel}-1`).value = leMotComplet.substring(0,1).toUpperCase(); //Prend en valeur la premiere lettre du mot à trouver
...
document.addEventListener("keydown", async function (event) {
  ...
  for(let i = 2; i < wordLength +1; i++){
    ...
    document.getElementById(`${essaiActuel}-${i}`).oninput = function() {
      ...
      if(str.length == 1){
        document.getElementById(`${essaiActuel}-${i}`).value = document.getElementById(`${essaiActuel}-${i}`).value.normalize("NFD").replace(/[\u0300-\u036f]/g, "");//Enleve l'accent saisi  
        document.getElementById(`${essaiActuel}-${i}`).value = document.getElementById(`${essaiActuel}-${i}`).value.toUpperCase(); //Met la lettre en majuscule
...
document.addEventListener("keydown", async function (event) {
  ...
  if (event.keyCode == 8) {
    ...
    while(i<wordLength+1 && !trouve){
      ...
      if (await document.getElementById(`${essaiActuel}-${i+1}`).value.length == 0) {
